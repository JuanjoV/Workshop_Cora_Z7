...
HEADERS = ["Hardware", "GoldenReference", "AbsoluteError", "PercentageError"]
ABS_TOL = 1
PER_TOL = 10
...
with open(CSV_FILE, 'w', newline='', encoding='utf8') as csvf, \
    ...
    # Skip headers if present (ajusta si tus CSV no tienen encabezado)
    next(ireader, None)
    next(rreader, None)
    ...
    for tst, (irow, rrow) in enumerate(zip(ireader, rreader)):
        ...
        writer.writerow([data_out, data_ref, abs_err, per_err])
